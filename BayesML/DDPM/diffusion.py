...
class DiffusionRunner:
    ...
    def calc_loss(self, clean_x: torch.Tensor, eps: float = 1e-5) -> Union[float, torch.Tensor]:
        ...
        t = self.sample_time(batch_size, eps)
        noise_x, eps = self.sample_noise_x(clean_x, t)

        loss = ((eps - self.model(noise_x, t)) ** 2).sum(dim=(1, 2, 3))
        loss = loss.mean()
    ...
    def set_conditional_sampling(
        ...
        def new_score_fn(x, t, y):
            ...
            """
            score = self.calc_score(x, t, y)

            posterior_score_T = score + classifier_grad_fn(x, t, y) / T
    ...
    def set_classifier(self, classifier: torch.nn.Module, T: float = 1.0) -> None:
        ...
        def classifier_grad_fn(x, t, y):
            ...
            with torch.enable_grad():
                ...
                logits = self.classifier(x, t)[torch.arange(x.shape[0]), y]
